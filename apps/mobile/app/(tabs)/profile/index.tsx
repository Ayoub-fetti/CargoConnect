...
export default function ProfileScreen() {
  ...
  const onSaveProfile = async () => {
    ...
    try {
      await driverService.updateProfile({
        ...
        phone: phone.trim(),
        licenseTypes: licenseTypes.split(',').map((item) => item.trim()).filter(Boolean),
        zone: zone.split(',').map((item) => item.trim()).filter(Boolean),
        isAvailable,
      });
  ...
  const avatarUrl = profile?.avatar && profile.avatar.startsWith('uploads')
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.avatarRow}>
            {avatarUrl ? (
              ...
              <View style={styles.avatarPlaceholder}>
                <Text style={styles.avatarPlaceholderText}>{(profile?.fullName || 'D')[0]}</Text>
              </View>
            )}
        ...
        <Pressable onPress={onSaveProfile} disabled={saving} style={styles.primaryButton}>
        ...
        <View style={[styles.header, { borderBottomWidth: 0, marginTop: 20 }]}>
          ...
          <Text style={[styles.headerTitle, { fontSize: 24 }]}>Mes Justificatifs</Text>
        </View>
        ...
        <View style={styles.card}>
          ...
          <TextInput 
            ...
            placeholderTextColor="rgba(255,255,255,0.2)"
          />
          <Pressable onPress={onUploadDocument} style={styles.secondaryButton} disabled={uploading}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={styles.docItem}>
                ...
                <Pressable onPress={() => driverService.deleteDocument(item._id).then(loadData)}>
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 80, height: 80, borderRadius: 24, backgroundColor: '#141414' },
  avatarPlaceholder: {
    width: 80,
    height: 80,
    borderRadius: 24,
  ...
  switchText: { color: 'rgba(255,255,255,0.7)', fontSize: 15, fontWeight: '500' },
  ...
  emptyText: { color: 'rgba(255,255,255,0.2)', textAlign: 'center', marginTop: 10 },
});