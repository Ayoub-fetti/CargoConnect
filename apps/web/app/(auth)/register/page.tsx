...
const steps = [
  { name: "companyName", label: "Entreprise",    type: "text",     placeholder: "Nom de votre entreprise", question: "Nom de votre entreprise ?" },
  { name: "email",       label: "Email",         type: "email",    placeholder: "vous@exemple.com",        question: "Votre adresse email ?" },
  { name: "password",    label: "Mot de passe",  type: "password", placeholder: "••••••••",                question: "Choisissez un mot de passe." },
  { name: "location",    label: "Localisation",  type: "text",     placeholder: "Paris, France",           question: "Où êtes-vous situé ?" },
];
...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({ companyName: "", email: "", password: "", location: "" });
  ...
  const goNext = () => {
    if (!form[currentField].trim()) { setError("Ce champ est requis."); return; }
    setError("");
    if (current === steps.length - 1) { handleSubmit(); return; }
    setDirection("next");
    setAnimating(true);
    setTimeout(() => { setCurrent((p) => p + 1); setAnimating(false); }, 300);
  };
  ...
  const handleKey = (e: React.KeyboardEvent) => { if (e.key === "Enter") goNext(); };

  if (done) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-white">
        <div className="text-center">
          <div className="text-6xl mb-6" style={{ animation: "pop 0.4s ease" }}>✓</div>
          <h2 className="text-3xl font-black text-black tracking-tight">Compte créé.</h2>
  ...
  return (
    <div className="flex min-h-screen bg-white">
      ...
      <div className="hidden lg:flex w-1/2 bg-black flex-col justify-between p-14">
        <Link href="/" className="text-lg font-black uppercase text-white" style={{ letterSpacing: "0.08em" }}>
          CargoConnect
        </Link>
        <div>
          <p className="text-4xl font-black text-white leading-tight tracking-tight">
            Rejoignez<br />la plateforme.
          </p>
          <p className="mt-4 text-sm text-gray-500 leading-relaxed max-w-xs">
            Créez votre compte en quelques secondes et commencez à gérer vos missions dès aujourd'hui.
          </p>
        </div>
        <p className="text-xs text-gray-600">© {new Date().getFullYear()} CargoConnect</p>
      </div>
      ...
      <div className="flex w-full lg:w-1/2 flex-col px-10 py-14 sm:px-16">
        ...
        {/* Mobile logo */}
        <Link href="/" className="lg:hidden text-base font-black uppercase text-black mb-10" style={{ letterSpacing: "0.08em" }}>
          CargoConnect
        </Link>
        ...
        <div className="w-full max-w-sm mx-auto mb-12">
          <div className="flex justify-between mb-3">
            {steps.map((s, i) => (
              <span key={s.name} className="text-xs font-semibold uppercase tracking-widest transition-colors duration-300"
          ...
          <div className="h-px w-full bg-gray-100 relative overflow-hidden">
            <div className="absolute top-0 left-0 h-full bg-black transition-all duration-500 ease-in-out"
        ...
        <div className="flex-1 flex flex-col justify-center max-w-sm w-full mx-auto">
          ...
          <div className="mt-12 flex items-center justify-between">
            ...
            <button onClick={goNext} disabled={loading}
              ...
              {loading ? "Création..." : current === steps.length - 1 ? "Créer mon compte →" : "Suivant →"}
            </button>
          </div>
        </div>
        ...
        <p className="mt-10 text-xs text-gray-400 text-center">
          Déjà un compte ?{" "}
          <Link href="/login" className="font-bold text-black hover:underline">Se connecter</Link>
        </p>
      </div>
