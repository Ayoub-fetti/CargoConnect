...
export default function AdminDashboard() {
  ...
  const activeRate =
    stats.users.activeDrivers + stats.users.activeCompanies > 0
      ? Math.round(
          ((stats.users.activeDrivers + stats.users.activeCompanies) /
            (stats.users.totalDrivers + stats.users.totalCompanies)) *
            100,
        )
      : 0;
  ...
  return (
    <div className="min-h-screen bg-white p-8 space-y-8">
      ...
      <div className="grid grid-cols-3 gap-px bg-gray-100 rounded-2xl overflow-hidden">
        {[
          {
            value: stats.users.totalDrivers + stats.users.totalCompanies,
            label: "Utilisateurs total",
            sub: `${stats.users.activeDrivers + stats.users.activeCompanies} actifs`,
          },
          {
            value: stats.missions.total,
            label: "Missions créées",
            sub: `${missionStatuses.length} statuts`,
          },
          {
            value: stats.applications.total,
            label: "Candidatures",
            sub: `${stats.subscriptions.active} abonnements actifs`,
          },
        ].map((item) => (
          <div
            key={item.label}
      ...
      <div className="grid grid-cols-4 gap-6">
        ...
        <div className="col-span-2 bg-black rounded-2xl p-6 flex flex-col justify-between">
          ...
          <div className="flex items-end justify-between mt-6">
            <div>
              ...
              <p className="mt-2 text-xs text-gray-500">
                actifs sur {stats.subscriptions.total}
              </p>
            </div>
          ...
          <div className="mt-6 h-1 w-full bg-gray-800 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${subRate}%`, transition: "width 1s ease" }}
            />
          </div>
        </div>
        ...
        {[
          {
            label: "Chauffeurs",
            total: stats.users.totalDrivers,
            active: stats.users.activeDrivers,
          },
          {
            label: "Entreprises",
            total: stats.users.totalCompanies,
            active: stats.users.activeCompanies,
          },
        ].map((item) => (
          <div
            key={item.label}
            className="border border-gray-100 rounded-2xl p-6 flex flex-col justify-between hover:border-black transition-colors duration-200"
          >
            <p className="text-xs font-semibold uppercase tracking-widest text-gray-300">
              {item.label}
            </p>
            <p className="mt-4 text-5xl font-black text-black leading-none">
              {item.total}
            </p>
            <div className="mt-4">
