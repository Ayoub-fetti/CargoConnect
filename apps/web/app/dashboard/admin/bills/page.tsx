    ...
    <div className="min-h-screen bg-white p-8 space-y-8">
      ...
      <div className="flex items-end justify-between border-b border-gray-100 pb-8">
        <div>
          ...
          <h1 className="text-5xl font-black text-black tracking-tight">Facturation</h1>
        </div>
        <p className="text-xs font-black text-black">
          {rows.length} <span className="font-medium text-gray-400">abonnements</span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-px bg-gray-100 rounded-2xl overflow-hidden">
        {[
          { label: "Revenus encaissés", value: `${totalRevenue.toLocaleString("fr-FR")} €` },
          ...
          <div key={item.label} className="bg-white px-8 py-6">
            <p className="text-xs font-semibold uppercase tracking-widest text-gray-300">{item.label}</p>
            <p className="mt-3 text-4xl font-black text-black leading-none">{item.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-3">
        ...
        {rows.map((row, i) => {
          ...
          return (
            <div key={i} className={`rounded-2xl border transition-colors duration-200 overflow-hidden ${isOpen ? "border-black" : "border-gray-100 hover:border-gray-300"}`}>
              ...
              >
                <div className="flex items-center gap-4">
                  {/* Avatar */}
                  <div className={`h-10 w-10 rounded-full flex items-center justify-center text-sm font-black transition-colors duration-200 ${isOpen ? "bg-black text-white" : "bg-gray-100 text-gray-600 group-hover:bg-black group-hover:text-white"}`}>
                    {row.company?.companyName?.charAt(0).toUpperCase() ?? "?"}
                  </div>
                  <div>
                    <p className="font-bold text-black text-sm">{row.company?.companyName ?? "—"}</p>
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex items-center gap-1.5">
                    <div className={`h-1.5 w-1.5 rounded-full ${row.status === "active" ? "bg-black" : "bg-gray-300"}`} />
                    <span className={`text-xs font-semibold ${row.status === "active" ? "text-black" : "text-gray-300"}`}>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-50">
                          {["Montant", "Statut", "Date", ""].map((h) => (
                            <th key={h} className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-widest text-gray-300">
                              {h}
                            </th>
                          ))}
                        </tr>
                      ...
                      <tbody>
                        {row.invoices.map((inv) => (
                          <tr key={inv.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors duration-100">
                            <td className="px-6 py-4 font-black text-black">
                              {inv.amount.toLocaleString("fr-FR")} {inv.currency.toUpperCase()}
                            </td>
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-1.5">
                                <div className={`h-1.5 w-1.5 rounded-full ${inv.status === "paid" ? "bg-black" : "bg-gray-300"}`} />
                                <span className={`text-xs font-semibold ${inv.status === "paid" ? "text-black" : "text-gray-400"}`}>
