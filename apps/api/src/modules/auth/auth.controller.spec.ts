...
describe('AuthController', () => {
  ...
  it('should register a company', async () => {
    mockAuthService.registerCompany.mockResolvedValue({ message: 'Registration successful. Please check your email to verify your account.' });
    const result = await controller.registerCompany({ email: 'test@test.com', password: '123456', companyName: 'Test' } as any);
    expect(result).toEqual({ message: 'Registration successful. Please check your email to verify your account.' });
  });

  it('should register a driver', async () => {
    mockAuthService.registerDriver.mockResolvedValue({ message: 'Registration successful. Please check your email to verify your account.' });
    const result = await controller.registerDriver({ email: 'd@d.com', password: '123', fullName: 'Driver' } as any);
    expect(result).toEqual({ message: 'Registration successful. Please check your email to verify your account.' });
  });

  it('should login', async () => {
    mockAuthService.login.mockResolvedValue({ access_token: 'token' });
    const result = await controller.login({ email: 'a@a.com', password: '123' });
    expect(result).toEqual({ access_token: 'token' });
  });

  it('should refresh token', async () => {
    mockAuthService.refreshToken.mockResolvedValue({ access_token: 'new_token' });
  ...
  it('should send forgot password email', async () => {
    mockAuthService.forgotPassword.mockResolvedValue({ message: 'If email exists, reset link has been sent' });
